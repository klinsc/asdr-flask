...
def predict():
    ...
    try:
        ...
        with open(f'{image_name}.csv', 'r') as f:
            ...
            # create a response object
            response = make_response(
                csvFile, 200, {'Content-Type': 'text/csv'})
...
async def validate_bays(drawing_components_df, drawing_type="Main&Transfer", test=True):
    ...
    try:
        ...
        for index, row in drawing_components_df.loc[(
                drawing_components_df["name"] == "115_vt_1p") | (drawing_components_df["name"] == "115_cvt_1p") | (drawing_components_df["name"] == "115_vt_3p") | (drawing_components_df["name"] == "115_cvt_3p")].iterrows():
            if (row["count"]):
                drawing_components_df.loc[index, "count"] -= 1
                break
        ...
        line_types_df.loc[line_types_df["name"] == "115_incoming", "count"] += drawing_components_df.loc[(
            drawing_components_df["name"] == "115_vt_1p") | (drawing_components_df["name"] == "115_cvt_1p") | (drawing_components_df["name"] == "115_vt_3p") | (drawing_components_df["name"] == "115_cvt_3p"), "count"].sum()
        ...
        line_types_df.loc[line_types_df["name"] == "22_incoming", "count"] += drawing_components_df.loc[(
            drawing_components_df["name"] == "v_m"), "count"].sum()
...
async def validate_components(drawing_components_df, line_types_df, drawing_type="Main&Transfer"):
    try:
